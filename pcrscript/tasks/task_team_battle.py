...
from ..game_ui.team_battle import Boss, boss_detail, can_repeat, map_bosses, map_ready, parse_remaining_time, priority, recommendation_time
...
class TeamBattle(TimeLimitTask):
    ...
    def open_boss(self, boss: Boss):
        ...
        if visible is None:
            raise EventUIError('目标首领已变化，停止使用旧模拟战结果')
    ...
    def recommendation(self, wanted=None):
        ...
        for _ in range(12):
            ...
            for button in buttons:
                ...
                if estimate is None or 2*estimate+8 > 110:
            ...
            self.ui.swipe((500, 390), (500, 210))
            after = self.ui.capture()
    ...
    def prepare_boss(self, boss: Boss, simulate: bool):
        ...
        if (current.name, current.current_hp, current.maximum_hp) != (boss.name, boss.current_hp, boss.maximum_hp):
            self.ui.expect_click('取消', (560, 425, 755, 505), exact=True)
            raise EventUIError('首领身份或血量已变化，旧模拟战无效')
        ...
        for _ in range(self.options['max_simulations']-len(self.report['simulations'])):
            ...
            if not simulation['win']:
                if wanted is not None:
                    return None
                continue
            if wanted is None and boss.full and not can_repeat(simulation['remaining']):
                continue
    ...
    def run(self, event: Event | None = None):
        try:
            ...
            if screen is None:
                self.report['status'] = 'unavailable'
                return self.report
            for _ in range(self.options['max_real_attacks']*2):
                self.check_deadline()
                screen = self.map()
                ...
                if not bosses:
                    self.report['pending'].append('没有可见的团队战首领')
                    break
                if self.carry:
                ...
                for boss in ordered:
                    ...
                    if selected:
                        break
                if selected is None:
                    self.report['pending'].append('当前首领均无可核验的五人高级推荐队伍或模拟战胜利')
                    break
