...
GUI 的“每日日常”保存一组按顺序执行的任务；“按需专项”每次只处理一个目标。两者使用同一份 Python 任务实现与运行记录。首次设置见 [GUI 指南](desktop.md)。

## 一、日常任务

点击“＋ 添加任务”后选择类型并填写参数。添加后选中新行，修改参数或用上移/下移调整顺序；取消启用会保留该项但不执行。保存配置后运行。相同任务可以出现多次，例如扫荡前后分别领取首页任务奖励。
...
| 首页任务领奖 | `get_quest_reward` | 领取任务页面奖励，可能包含体力 |
...
| 商店购买 | `shop_buy` | JSON 购买规则，会消耗对应货币；如 `{"1":[-1]}` 表示普通商店全选 |

示例顺序见 [daily.example.yml](examples/daily.example.yml)。GUI 新建的是空白配置，不自动安排消费或培养。需要首页的任务由统一调度返回首页，不必穿插 `tohomepage`；不能识别的弹窗保留现场。

命令行通过 `daily_config.yml` 的 `Task` 配置列表，完整日常运行 `daily_task.py`。GUI 编辑第一个任务组，保留已有账号与其他组，但不提供账号登录或多组切换。
...
打开 `prepare_only` 可只获取并解析来源，深域需指定 `sources.stage` 和 `sources.element`。当前视频解析、多刀地下城规划和全局培养核验仍有待办，不能保证生成每个关卡的完整作业；见[范围与交接](video-strategies.md)。Agent 接管时复核报告与原始证据。
