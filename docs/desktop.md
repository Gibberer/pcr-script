...
3. “每日日常”默认加载上次使用的有效配置，失效时选择工程内 `daily_config.yml`；都不存在则显示空白草稿。可打开其他 YAML 或新建空白配置，在“日常选项”编辑随配置保存的选项。雷电目录由环境设置管理。
4. 日常编辑器仅提供每日工作与可根据活动情报调度的任务，支持启用、排序、移除和单项执行。“按需专项”独立提供剧情清理、地下城首通、来源搜索、全角色强化、普通冒险、驾车游及返回首页，显示任务说明、起始页面要求与独立的本次参数/选项。专项不能通过新增按钮加入日常；旧配置已有专项仍保留并标注，不自动迁移或删除。
...
6. “开始今日日常”运行已加载配置中全部启用项，运行前保存；取消新配置保存不会开始执行。“单独执行此日常项”只执行选中日常项。“执行本次专项”只传递所选专项的参数、对应配置段和运行环境，不保存或改写日常列表。专项初值来自已加载配置及程序默认值，切换专项会重新初始化表单。本次编辑与日常选项相互隔离。运行状态标注日常、单项日常或专项；队伍来源搜索无需连接模拟器。
