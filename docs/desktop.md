...
适用 Windows 10 1903+ / Windows 11，使用系统 .NET Framework 4.8，不需要安装 .NET SDK。打开 GUI 不需要 Python；执行任务需要 Windows x64 Python（推荐 3.12）及雷电模拟器，下载/更新源码需要 Git。
...
同一模拟器同时只能由一个自动化进程操作。使用项目的雷电后台驱动，不需要 ADB。
...
“日常选项”按活动/功能折叠分组，与日常配置一起保存。雷电目录由环境设置统一管理。旧配置里已有的专项任务仍保留并标注，不会自动删除；新增日常项只展示日常类别。
...
地下城与深域可启用“仅获取并解析攻略（不操作游戏）”。当前视频布局、培养与辅助条件识别存在明确边界，未知条件保留待办，不保证任意视频都能生成可执行方案。详见 [任务指南](tasks.md#地下城与深域的来源处理) 和 [视频解析交接](video-strategies.md)。
...
“环境设置”集中管理工程、Python、雷电和源码版本。源码下载/更新选项默认折叠；更新前检查运行锁、工作区修改和分支，只允许快进，不覆盖本地源码。更新完成后重新安装依赖并加载配置。
