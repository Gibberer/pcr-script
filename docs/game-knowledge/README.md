...
Windows 前台控制终端及离线验证说明见 [GUI 文档](../desktop.md)。GUI 不新增游戏规则；游戏实测与仍待验证的范围以各专项记录和[待验证清单](pending-validation.md)为准。

**2026-09-19 用户纠正的实现边界：** Agent 操作只用于分析验证。头像获取/更新与当期队伍方案获取/生成必须纳入正式自动化，不得依赖 Agent 手工准备。真实配置和方案不提交；本地缓存可跑通不等于空缓存端到端完成。2026-09-23 公共头像自动获取/建库及有限视频布局的正式解析已接通，完整培养、辅助条件与地下城路线仍有明确待办，见[视频解析与复核](../video-strategies.md)、待验证表与 `AGENTS.md`。Agent 接管时必须复核脚本结果，不能把人工读图成功当作脚本支持。
...
- [深域关卡](abyss.md)：属性地图、NEXT定位、有限本地试打与通用来源搜索。
...
- [角色好感度任务](../character-bond.md)：使用现有礼物提升好感度，逐篇读取解锁的角色剧情与属性奖励。
