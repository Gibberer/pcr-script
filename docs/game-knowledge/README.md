...
**2026-09-19 用户纠正的实现边界：** Agent 操作只用于分析验证。头像获取/更新与当期队伍方案获取/生成必须纳入正式自动化，不得依赖 Agent 手工准备。真实配置和方案不提交；本地缓存可跑通不等于空缓存端到端完成。地下城来源自动搜索已接通并实测空缓存；从候选视频到可执行队伍的解析/规划及头像自动获取仍未接通，见待验证表与 `AGENTS.md`。
