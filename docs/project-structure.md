...
| [task_dungeon_sources.py](../pcrscript/tasks/task_dungeon_sources.py) | `dungeon_sources`（可脱离模拟器检索） |
...
`tasks/dungeon_sources.py` 为正式共享的匿名来源搜索/身份核验/缓存能力，不独立注册；`dungeon_sources` 与缺方案的首通入口均调用它。候选来源不是战斗方案。
