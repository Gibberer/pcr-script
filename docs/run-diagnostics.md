...
- `status.json`：PID、心跳、运行/暂停/结束状态、当前步骤、结构化任务/操作进度、最近操作、截图时间、错误次数、控制请求确认 ID。
...
Windows GUI 通过 `pcrscript/desktop.py` 复用同一状态文件与控制协议，另支持协作式 `stop`：驱动边界或公共时钟等待时抛出取消信号，退出后状态为 `cancelled`，取消不会记为错误。暂停状态也可停止；系统调用阻塞时仍需等待返回。CLI 原有暂停/恢复/现场命令保持兼容，GUI 使用见 [图形控制台](guides/desktop.md)。

2026-09-23 实测：GUI 停止请求使运行记录 `9cec8124-4f0d-43e8-84cb-139f83562bf9` 在最后一次截图后进入 `cancelled`，无 Python 进程残留；旧 GUI 仍用当前时间刷新已结束任务的「耗时」。显示已改为结束时心跳时间，修复后的 GUI 待正常使用时确认。
...
任务实例的执行记录写在 `tasks/<序号>-<任务名>/result.json`，新任务的业务报告和必要消费证据同目录归档；旧任务只记录调度结束时仍可能是 `finished`，不能据此认定资源已核验。结果状态与执行路径见[任务架构](task-architecture.md)。

2026-09-21 的一次完整日常在雷电 960×540 下用时 671 秒、474 次截图、245 次点击，运行记录为 `cache/daily/runs/20260921-162528-33264-b67a00/`。该次正常结束，未请求暂停。固定等待、返回首页与旧任务缺少业务后置核验仍有优化空间；实机暂停/恢复与未知页面处理见[待验证清单](pending-validation.md)。这些运行数字只用于定位耗时，不代表当前版本的性能承诺。

离线回归覆盖暂停确认、恢复时画面变化拦截、停止取消、异常现场、日志分流关闭后的退出处理及并发运行互斥：
