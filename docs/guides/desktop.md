...
2. 下载范围默认“仅核心运行文件”，包含 Python 执行代码、游戏图片、依赖清单和 GUI 默认选项文件。要把该目录交给 AI Agent 开发并继续用 GUI 运行，请选“完整仓库”。源码下载默认使用 `master` 分支；旧版保存的默认开发分支会自动改为 `master`，用户明确设置的其他分支保留。
3. 选好有效工程目录后即可进入控制台。Python、依赖和设备都可稍后在“环境设置”准备；无须在首次引导选择 YAML。缺少 Python 或依赖时，加载配置或运行任务会显示错误，不会把用户留在首次引导。已有依赖的版本与 `requirements.txt` 不同时，也不会单凭版本差异拒绝运行。
4. 使用雷电后台驱动时选择包含 `ldconsole.exe` 的安装目录。使用 ADB 时留空雷电目录，填写 `adb`（已加入 PATH）或选择 Android Platform Tools 的 `adb.exe`；运行前确认该程序可运行。连接多个 ADB 设备时，填写 `adb devices` 显示的目标序列号；只有一个已授权设备时可留空。
5. 自动检测已有 Python，或使用引导里的官网下载入口安装 Python 3.12 x64 后重新检测。点击“创建环境 / 安装依赖”，GUI 在工程中创建 `.venv` 并安装依赖，首次需要联网。

同一设备同时只能由一个自动化进程操作。雷电目录非空时使用雷电驱动；窗口输入失败时驱动会尝试 ADB 回退。目录留空时直接使用 ADB。界面识别使用 960×540 设计坐标，操作按实际截图宽高分别换算；当前任务流程只在雷电 960×540 实机验证，其他 Android 设备、分辨率和非 16:9 布局仍需实测。

雷电窗口句柄不可用、但模拟器仍通过 ADB 连接时，可在配置的 `Extra` 中留空 `dnpath`、指定 `adb_serial`，并设置 `adb_unicode_console_path` 为本机 `ldconsole.exe` 的绝对路径、`adb_unicode_console_index` 为实例序号。此时截图和点击走 ADB，中文角色名通过雷电控制台输入；程序仍会回读搜索词并核验角色。未配置控制台时，ADB 遇到中文编队会逐页扫描并核验角色头像。此组合只在雷电 960×540 验证，其他设备需单独验证。

## 2. 运行与调整每日日常

![日常任务列表与参数编辑](images/gui-daily.png)

准备好 Python 依赖和设备后，进入“每日日常”点击“开始今日日常”即可。新工程会加载根目录 `runtime_defaults.yml` 中已经排好的日常任务，首次保存时生成忽略的 `daily_config.yml`，不修改共用默认文件。列表中的快捷扫荡使用游戏预设 2；如需改用其他预设，可在运行前调整这一项。
...
“运行状态”从运行记录读取任务与操作进度，分别显示进度条、当前状态和耗时。终端输出收在默认折叠的“详细日志（排错）”中；命令行的动态进度条不会写入 GUI 日志。控制按钮随状态启用：运行时可请求暂停，确认暂停后才能继续；停止也需要等待确认。阻塞的系统或网络请求可能延迟响应，不能把请求已发出视为已经停止。停止脚本不会暂停游戏中的战斗。
