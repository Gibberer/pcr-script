...
GUI 的“每日日常”会按顺序执行预设列表；配置好 Python 依赖和设备后即可点击“开始今日日常”。“按需专项”每次只处理一个目标。两者使用同一份 Python 任务实现与运行记录。首次设置见 [GUI 指南](desktop.md)。
...
| 团队战 | `team_battle` | 活动期间在第二次快捷扫荡后执行；逐条筛高级推荐中的可用五人，使用前取消该行「保存队伍」勾选，读取参考伤害和首领当前生命值，装好特别装备并立即模拟。达标后使用保留编队实战，未达标才换下一队；击杀且返还时间足够时尝试延长挑战，最多三次普通挑战。见[玩法与验证范围](../game-knowledge/team-battle.md)。 |
...
团队战需要只验证模拟战时，在本次使用的 YAML 的 `TeamBattle` 段设置 `simulation_only: true`。任务完成一场模拟战后保存参考伤害、实际伤害与达标结果，返回地图，不进入实战编队；常规日常不设置此项。
